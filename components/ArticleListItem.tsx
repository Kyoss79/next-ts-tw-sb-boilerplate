...
const ArticleListItem: FunctionComponent<ArticleListItemProps> = ({
  title,
  subtitle,
  text,
  datePublished,
  ...
  return (
    <>
      <article className="border rounded-lg p-4 bg-gray-700 text-white">
        ...
        <p className="py-2 my-2 border-b border-t text-gray-200 text-sm">
          {text}
        </p>
        <div className="flex justify-between text-gray-400 text-sm">
          <div>
            by <span className="text-gray-100">{author}</span> on{' '}
            {datePublished.toLocaleDateString('en-US', {
              dateStyle: 'long'
            })}
          </div>
          <div>
            <span className="mr-2">💬</span> {comments} comments
          </div>
        </div>
      </article>
    </>
